...
@st.cache_data
def load_data() :
    return pd.read_csv(r'C:\Users\dinda\Documents\Bootcamp\my_portfolio\dataset\data_dummy_retail_store.csv')


# load data penjualan
df_sales = load_data()
df_sales.columns = df_sales.columns.str.lower().str.replace(' ', '-')
...
st.markdown("Dashboard interaktif ini menyediakan gambaran umum performa penjualan, trend, dan model prediksi")

st.markdown("----") # garis pembatas

# ---- sidebar untuk filter2 ----
...
if pilihan_halaman == "Overview Dashboard" :
    ...
    if len(date_range) == 2 :
        ...
        # kalau filter datenya belum tuntas
...
if pilihan_halaman == "Overview Dashboard" :
    ...
    with col3 :
        st.metric(label="Rata-rata Nilai Pesanan", value = f"Rp {avg_order_value:,.1f}")
    with col4 :
    ...
    with col_vis1 :
        ...
        top_products_sold = filtered_df.groupby('produk')['total_penjualan'].sum().nlargest(10).reset_index() # agregat total penjualan
